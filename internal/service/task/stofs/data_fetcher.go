...
/*
   ...
   @Author: khaosles
   @Time: 2023/8/25 00:01
   @Desc:
*/

type GfsStofsDataFetcher struct {
	node2.BaseNode
}

func NewGfsStofsDataFetcher(id, proxy string, cache node2.Cache) *GfsStofsDataFetcher {
	var n GfsStofsDataFetcher
	n.Id = id
	n.Proxy = proxy
...
func (n *GfsStofsDataFetcher) Execute() *node2.Result {
	...
	for _, uri := range uris {
		// 下载地址
		url = strings.Replace(uri.Url, "{TTT}", idsplit[2], -1)
		// 文件名称
		filename = strings.Replace(uri.Filename, "{TTT}", idsplit[2], -1)
		// 临时下载路径
